...
- Dentro do loop, ocorre o processo de verificar se o caracter `?` caso for o loop se encerra, ocorre a verificao se o caracter e `*` caso for o processo de reinicializacao e feito, este que ja foi explicado acima. Apos verificar se a posicao atual e algum destes caracteres comeca a verificacao de qual e a posicao atual, pois a posicoes que existem excecoes no momento de adicionar suas vizinhas a fila. Dentro de cada `if` ou `else if` que sao as excessoes tratadas e chamada a funcao `movimento_x_y()` antes de ser chamada e somado ou subtraido 1 da linha e coluna quando necessario, a soma ou subtracao e feita de maneira a respeitar a condicao de movimento do BFS para que dentro da funcao ocorra a comparacao com caracteres da posicao vizinha que esta sendo analisada, apos finalizar a funcao e somado ou subtraido 1 da linha e coluna para voltar ao valor da posicao atual. 
- A funcao `movimento_x_y()` e resposavel por verificar qual caracter esta na posicao vizinha e enfileira-los quando necessario.
